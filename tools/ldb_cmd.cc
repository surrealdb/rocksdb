...
CloudManifestDumpCommand::CloudManifestDumpCommand(
    const std::vector<std::string>& /*params*/,
    const std::map<std::string, std::string>& options,
    const std::vector<std::string>& flags)
    : LDBCommand(options, flags, false /* is_read_only */,
...
void CloudManifestDumpCommand::DoCommand() {
  ...
  if (!io_st.ok()) {
    std::cerr << "Error in reading cloud manifest file: "
              << cloud_manifest_file << "; Error: " << io_st.ToString()
              << std::endl;
    return;
  }
  ...
  if (!st.ok()) {
    std::cerr << "Error in loading the cloud manifest file %s"
